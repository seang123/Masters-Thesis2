...
class NIC(tf.keras.Model):
    ...
    def predict(self, betas, hidden, carry, start_seq, max_len, units, tokenizer, training=False):
        betasA = betas[:betas.shape[0]//2]
        betasB = betas[betas.shape[0]//2:]
        hiddenA = hidden[:hidden.shape[0]//2, :]
        hiddenB = hidden[hidden.shape[0]//2:, :]
        ...
        startA = start_seq[:start_seq.shape[0]//2]
        startB = start_seq[start_seq.shape[0]//2:]

        # Call the predict_attention function with different encoders
        outputA, attn_scoresA = self.predict_attention(self.dense_in_a, betasA, hiddenA, carryA, startA, max_len, units, tokenizer, training=False)
        outputB, attn_scoresB = self.predict_attention(self.dense_in_b, betasB, hiddenB, carryB, startB, max_len, units, tokenizer, training=False)

        return [outputA, outputB], [attn_scoresA, attn_scoresB]

    def predict_attention(self, enc_func, betas, a0, c0, start_seq, max_len, units, tokenizer, training=False):
        """ Takes a encoder function as a parameter """
        features = enc_func(betas, training=training)
        ...
        for i in range(max_len):
            context, attention_score, _= self.attention(a, features, training=training)
