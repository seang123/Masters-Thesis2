...
import sys
sys.path.append("/home/hpcgies1/Masters-Thesis/AttemptFour/Model")
...
"""
    Multi-subject model that should work with all subjects together
    It takes a string which defines which encoder to use
        the string should be the name of the encoder function
        ie. self.encoder_1 = Dense() with string 'encoder_1'
"""
...
class NIC(tf.keras.Model):
    ...
    def __init__(self, groups, units, embedding_features, embedding_text, attn_units, vocab_size, max_length, dropout_input, dropout_features, dropout_text, dropout_attn, dropout_lstm, dropout_out, input_reg, attn_reg, lstm_reg, output_reg):
        ...
        encoders = ['in_1', 'in_2', 'in_3', 'in_4', 'in_5', 'in_6', 'in_7', 'in_8']
        self.encoders_dict = {}
        for e in encoders:
            setattr(self,
                    f'dense_{e}',
            ...
            self.encoders_dict[e] = getattr(self, f'dense_{e}')
        print("encoders dict:", len(self.encoders_dict))
    ...
    def call(self, data, enc_func, training=False):
        return self.call_attention(data, enc_func, training)

    @tf.function
    def call_attention(self, data, training=False):
        """ Forward pass | Attention model """
        features, text_input, a0, c0 = data

        """
        img_input = self.dropout_input(img_input, training=training)
        # Features from regions
        features = encoder_func(img_input, training=training)
        """
        """
        features = enc_func(features, training=training)
        """
    ...
    def build_model(self, data):
        ...
        x = self.dense_in_0(betas, training=False)
        x = self.dense_in_1(betas, training=False)
        _, _ = self.call_attention(
                (x, data[0][1], data[0][2], data[0][3]),
                training=False
        )
        return

    def _train_step(self, features, text, hidden, carry, target):
        """ Data holds the encoded features, and the text """

        prediction, attention_scores = self.call_attention(
                (features, text, hidden, carry),
                training=True
    ...
    def train_step(self, data):
        ...
        len_enc_dict = len(self.encoders_dict)

        with tf.GradientTape() as tape:
            # Call model on sample

            for i in tf.range(len_enc_dict):
                target = data[1][i] # (batch_size, max_length, 5000)
                encoder_func = self.encoders_dict[i]
                features = encoder_func(data[0][i][0], training=True)
                cce, acc, attn = self._train_step(features, data[0][i][1], data[0][i][2], data[0][i][3], target)
                cross_entropy_loss += cce
                accuracy += accuracy
                attn_loss += attn

            # norm across subjects
            accuracy           /= len_enc_dict
            attn_loss          /= len_enc_dict
            cross_entropy_loss /= len_enc_dict
