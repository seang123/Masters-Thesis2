...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, batch):
        ...
        for i, key in enumerate(nsd_key):
            cur_sub = sub[i]
            with open(f"/home/hpcgies1/rds/hpc-work/NIC/Data/subj_{cur_sub}/betas_averaged/subj0{cur_sub}_KID{key}.npy", "rb") as f:
                betas_batch[i, :] = np.load(f)
            """
            if sub[i] == '1':
                with open(f"/home/hpcgies1/rds/hpc-work/NIC/Data/subj_1/betas_averaged/subj01_KID{key}.npy", "rb") as f:
                    betas_batch[i, :] = np.load(f)
            else:
                with open(f"/home/hpcgies1/rds/hpc-work/NIC/Data/subj_2/betas_averaged/subj02_KID{key}.npy", "rb") as f:
                    betas_batch[i, :] = np.load(f)
            """
