...
_, _, test_keys_one = loader.get_nsd_keys('1')
print("test_keys:", test_keys_one.shape)

_, _, test_keys_two = loader.get_nsd_keys('2')
print("train_keys:", test_keys_two.shape)
...
test_pairs_one   = loader.create_pairs(test_keys_one, subj='1', single=True)
print(f"test_pairs : {len(test_pairs_one)}")
test_pairs_two   = loader.create_pairs(test_keys_two, subj='2', single=True)
print(f"test_pairs : {len(test_pairs_two)}")
test_pairs = [test_pairs_one, test_pairs_two]
print("test_pairs:", len(test_pairs))

_, _, test_betas_one = loader.load_split_betas('1')
_, _, test_betas_two = loader.load_split_betas('2')
print("test_betas:", test_betas_one.shape)
print("test_betas:", test_betas_two.shape)

test_betas = np.stack((test_betas_one, test_betas_two), axis=0)
...
def eval_model():
    ...
    for i in tqdm.tqdm(range(0, len(data_generator)+1)):
        ...
        #outputs, outputs_raw, attention_scores, _ = model.greedy_predict(features, tf.convert_to_tensor(a0), tf.convert_to_tensor(c0), start_seq, config['max_length'], config['units'], tokenizer)
        outputs, attention_scores = model.predict(features, tf.convert_to_tensor(a0), tf.convert_to_tensor(c0), start_seq, config['max_length'], config['units'], tokenizer) # (list, list)
    ...
    for i in range(len(all_outputs)):
        outputsA.append(all_outputs[i][0])
        outputsB.append(all_outputs[i][1])

    outputsA = np.concatenate((outputsA), axis=0)
    outputsB = np.concatenate((outputsB), axis=0)
    outputs = np.stack((outputsA, outputsB), axis=0) # (2, 515, 15, 1)

    # Attention
    attention_scoresA = []
    attention_scoresB = []
    for i in range(len(all_attention_scores)):
        attention_scoresA.append(all_attention_scores[i][0])
        attention_scoresB.append(all_attention_scores[i][1])

    attention_scoresA = np.swapaxes(np.concatenate(attention_scoresA, axis=1), 0, 1)
    attention_scoresB = np.swapaxes(np.concatenate(attention_scoresB, axis=1), 0, 1)
    print("attn scores A", attention_scoresA.shape)
    attention_scores = np.stack((attention_scoresA, attention_scoresB), axis=0) # (2, 515, 15, 360, 1)
