...
GUSE_model_path = '/home/hpcgies1/Masters-Thesis/AttemptFour/GUSE/GUSE_model'
nsd_keys = './TrainData/subj02_conditions2.csv'
...
def load_all_captions():
    ...
    ls = defaultdict(list)
    with open(f"{captions_path}/all_captions.txt", "r") as f:
...
def get_captions(keys: list):
    caps = []
    nsd_to_caps = load_all_captions()
    keys = set(keys)
    for k, v in nsd_to_caps.items():
        if k not in keys:
            continue
        temp = []
        for cap in v:
            ...
            cap = " ".join(cap)
            temp.append(cap)
        assert len(temp) == 5, f"key:{key} - {len(captions)} | doesn't have 5 captions"
        caps.append(temp)
    return caps

def get_captions_old(keys: list):
...
if __name__ == '__main__':
    ...
    print(f"time to encode: {(time.time() - start_t):.2f}")
    print("guse output shape:", guse.shape)

    with open("/home/hpcgies1/rds/hpc-work/NIC/Data/GUSE/all_keys_guse..npy", "wb") as f:
        np.save(f, guse)
