...
#
# subj 4 has 30 sessions
# subj 6 has 32 sess
# subj 8 has 30 sess

subject_number = '3'
subject = f"subj0{subject_number}"
#data_dir = f"/fast/seagie/data/subj_{subject_number}/"
data_dir = f"/home/hpcgies1/rds/hpc-work/NIC/Data/subj0{subject_number}/"
nsd_dir = "/home/hpcgies1/rds/hpc-work/NIC/NSD/"
n_sessions = 32 # 40
targetspace = 'fsaverage'

response = pd.read_csv(f"{nsd_dir}/nsddata/ppdata/{subject}/behav/responses.tsv", sep='\t', header=0)
stim_info_merged = pd.read_csv(f"{nsd_dir}/nsddata/experiments/nsd/nsd_stim_info_merged.csv")
...
test_cond = ngd.get_conditions_515(nsd_dir)

shared_idx = np.zeros_like(sample)
test_idx = np.zeros_like(sample)
...
df_dict = {'nsd_key': sample, 'is_shared': shared_idx, 'is_test': test_idx}
df = pd.DataFrame(data=df_dict)
df.to_csv(f'./TrainData/{subject}_conditions2.csv', index=False)
...
if np.sum(good_vertex) != len(good_vertex):
    ...
    """
    np.save(f"{data_dir}/betas_averaged/all_betas.npy", betas_mean)
    print(f"all betas saved to single file: {data_dir}/betas_averaged/all_betas.npy")
