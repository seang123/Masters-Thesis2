...
from Model import ms3_NIC as lc_NIC
...
"""
    For many subject training - currentl 1, 2, 5, 7 - with Model/ms3_NIC.py and DataLoaders/data_generator_many_subs.py
"""
...
subprocess.run(["cp", "./Model/ms3_NIC.py", f"{run_path}/ms3_NIC.py"], shell=False, check=True)
subprocess.run(["cp", "./Model/layers.py", f"{run_path}/layers.py"], shell=False, check=True)
subprocess.run(["cp", "./Model/attention.py", f"{run_path}/attention.py"], shell=False, check=True)
...
train_pairs = np.array([train_pairs_one, train_pairs_two])
val_pairs = np.array([val_pairs_one, val_pairs_two])
print("combined train pairs:", train_pairs.shape)
print("combind val pairs:", val_pairs.shape)
...
# Stack betas
train_betas = np.stack((train_betas_one, train_betas_two), axis=0)
...
def dotfit():
    # .fit() doesn't work with variable batch-size
...
def custom_train_loop():
    ...
    model.build_model(train_generator[0])
    print("model built !")
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        # Progress bar
        pb_len = len(train_generator.batches)
        pb_len_val = len(val_generator.batches)
        pb = Progbar(pb_len)#, stateful_metrics=['loss', 'l2', 'accuracy'])
        pb2 = Progbar(pb_len_val)#, stateful_metrics=['val-loss', 'val-l2', 'val-accuracy'])

        # Training
        for (batch_nr, data) in enumerate(train_generator):
            callbacks.on_batch_begin(epoch, logs=logs)
            # encoder
            encoder_name = data[0][-1]
            encoder_func = getattr(model, encoder_name)

            losses = model.train_step(data, encoder_func)
